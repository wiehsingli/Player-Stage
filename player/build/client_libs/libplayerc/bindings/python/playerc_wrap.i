...
typedef struct
{
    ...
    %extend {
        uint8_t __getitem__(int index) {return $self->actual[index];}
        void __setitem__(int index,uint8_t value) {$self->actual[index]=value;}
    }
} uint8_tArray;
...
typedef struct
{
  ...
  int source;
	...
	%extend
	{
		...
		int set_source (int source, const char *norm);
		int get_source (void);
		int get_image (void);
		void copy_image (void * dst, size_t dst_size);
		unsigned get_pixel_component (unsigned int x, unsigned int y, int component);
	}
} playerc_camera;
...
%header
%{
	#define new_playerc_coopobject playerc_coopobject_create
	#define del_playerc_coopobject playerc_coopobject_destroy
	typedef playerc_coopobject_t playerc_coopobject;
%}
...
typedef struct
{
  ...
  uint8_tArray parameters;

	%extend
	{
		playerc_coopobject (playerc_client *client, int index);
		void destroy(void);
		int subscribe (int access);
		int unsubscribe (void);
		int send_position (uint16_t node_id, uint16_t source_id, player_pose2d_t pos, uint8_t status);
		int send_data (int node_id, int source_id, int data_type, int data_size, unsigned char *extradata);
		int send_cmd (int node_id, int source_id, int cmd, int parameters_size, unsigned char *parameters);
		int send_req (int node_id, int source_id, int req, int parameters_size, unsigned char *parameters);
	}
} playerc_coopobject;
  ...
  player_color_t color;
	...
	%extend
	{
		...
		int draw_polyline (player_point_2d_t pts[],
             int count );
		int draw_multiline (player_point_2d_t pts[],
             int count );
...
typedef struct
{
  ...
  playerc_localize_particle *particles;
	...
	%extend
	{
		...
		int subscribe (int access);
		int unsubscribe (void);
		int set_pose (double pose[3], double cov[6]);
...
typedef struct
{
  ...
  double waypoint_distance;
	...
	%extend
	{
		...
		int set_cmd_start (double sx, double sy, double sa);
...
typedef struct
{
	...
	%extend
	{
		...
		int reset_euler (float roll, float pitch, float yaw);
	}
} playerc_imu;
