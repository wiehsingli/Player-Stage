...
#include </home/wilson/ps/player/build/libplayerinterface/playerxdr.h>
#include <string.h>
...
{   if(xdr_u_int(xdrs,&msg->time_sec) != 1)
  ...
  if(xdr_int(xdrs,&msg->altitude) != 1)
    return(0);
  if(xdr_double(xdrs,&msg->speed) != 1)
    return(0);
  if(xdr_double(xdrs,&msg->course) != 1)
    return(0);
...
{   if(xdr_player_pose2d_t(xdrs,&msg->mean) != 1)
    return(0);
  if(xdr_vector(xdrs, (char*)&msg->cov, 6, sizeof(double), (xdrproc_t)xdr_double) != 1)
    return(0);
...
int xdr_player_camera_source_t (XDR* xdrs, player_camera_source_t * msg)
{   if(xdr_u_int(xdrs,&msg->norm_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
    if((msg->norm = malloc(msg->norm_count*sizeof(char))) == NULL)
      return(0);
  }
  {
    char* norm_p = msg->norm;
    if(xdr_bytes(xdrs, (char**)&norm_p, &msg->norm_count, msg->norm_count) != 1)
      return(0);
  }
  if(xdr_int(xdrs,&msg->source) != 1)
    return(0);
  return(1);
}
int 
player_camera_source_pack(void* buf, size_t buflen, player_camera_source_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_camera_source_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_camera_source_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_camera_source_t_copy(player_camera_source_t *dest, const player_camera_source_t *src)
{      
  ...
  if(src == NULL)
    return(0);
  size += sizeof(uint32_t)*1;
  memcpy(&dest->norm_count,&src->norm_count,sizeof(uint32_t)*1); 
  if(src->norm != NULL && src->norm_count > 0)
  {
    if((dest->norm = malloc(src->norm_count*sizeof(char))) == NULL)
      return(0);
  }
  else
    dest->norm = NULL;
  size += sizeof(char)*src->norm_count;
  memcpy(dest->norm,src->norm,sizeof(char)*src->norm_count); 
  size += sizeof(int32_t)*1;
  memcpy(&dest->source,&src->source,sizeof(int32_t)*1); 
  return(size);
}
void player_camera_source_t_cleanup(const player_camera_source_t *msg)
{      
...
player_camera_source_t * player_camera_source_t_clone(const player_camera_source_t *msg)
{      
  player_camera_source_t * clone = malloc(sizeof(player_camera_source_t));
  if (clone)
    player_camera_source_t_copy(clone,msg);
  return clone;
}
void player_camera_source_t_free(player_camera_source_t *msg)
{      
  player_camera_source_t_cleanup(msg);
  free(msg);
}
unsigned int player_camera_source_t_sizeof(player_camera_source_t *msg)
{
  ...
  if(msg == NULL)
    return(0);
  size += sizeof(uint32_t)*1; 
  size += sizeof(char)*msg->norm_count; 
  size += sizeof(int32_t)*1; 
  return(size);
}
...
{   if(xdr_u_int(xdrs,&msg->col) != 1)
  ...
  if(xdr_u_int(xdrs,&msg->data_count) != 1)
    return(0);
  if(xdr_char(xdrs,&msg->data_range) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
...
{      
  ...
  memcpy(&dest->data_range,&src->data_range,sizeof(int8_t)*1); 
  if(src->data != NULL && src->data_count > 0)
  {
...
{   if(xdr_double(xdrs,&msg->waypoints_distance) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->waypoints_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
...
{      
  ...
  if(src == NULL)
    return(0);
  size += sizeof(double)*1;
  memcpy(&dest->waypoints_distance,&src->waypoints_distance,sizeof(double)*1); 
  size += sizeof(uint32_t)*1;
...
int xdr_player_graphics2d_cmd_multiline_t (XDR* xdrs, player_graphics2d_cmd_multiline_t * msg)
{   if(xdr_u_int(xdrs,&msg->points_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
...
int 
player_graphics2d_cmd_multiline_pack(void* buf, size_t buflen, player_graphics2d_cmd_multiline_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_graphics2d_cmd_multiline_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_graphics2d_cmd_multiline_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_graphics2d_cmd_multiline_t_copy(player_graphics2d_cmd_multiline_t *dest, const player_graphics2d_cmd_multiline_t *src)
{      
...
void player_graphics2d_cmd_multiline_t_cleanup(const player_graphics2d_cmd_multiline_t *msg)
{      
...
player_graphics2d_cmd_multiline_t * player_graphics2d_cmd_multiline_t_clone(const player_graphics2d_cmd_multiline_t *msg)
{      
  player_graphics2d_cmd_multiline_t * clone = malloc(sizeof(player_graphics2d_cmd_multiline_t));
  if (clone)
    player_graphics2d_cmd_multiline_t_copy(clone,msg);
  return clone;
}
void player_graphics2d_cmd_multiline_t_free(player_graphics2d_cmd_multiline_t *msg)
{      
  player_graphics2d_cmd_multiline_t_cleanup(msg);
  free(msg);
}
unsigned int player_graphics2d_cmd_multiline_t_sizeof(player_graphics2d_cmd_multiline_t *msg)
{
...
int xdr_player_imu_reset_euler_config_t (XDR* xdrs, player_imu_reset_euler_config_t * msg)
{   if(xdr_player_pose3d_t(xdrs,&msg->orientation) != 1)
    return(0);
  return(1);
}
int 
player_imu_reset_euler_config_pack(void* buf, size_t buflen, player_imu_reset_euler_config_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_imu_reset_euler_config_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_imu_reset_euler_config_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_imu_reset_euler_config_t_copy(player_imu_reset_euler_config_t *dest, const player_imu_reset_euler_config_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_imu_reset_euler_config_t));
  return sizeof(player_imu_reset_euler_config_t);
} 
void player_imu_reset_euler_config_t_cleanup(const player_imu_reset_euler_config_t *msg)
{
} 
player_imu_reset_euler_config_t * player_imu_reset_euler_config_t_clone(const player_imu_reset_euler_config_t *msg)
{      
  player_imu_reset_euler_config_t * clone = malloc(sizeof(player_imu_reset_euler_config_t));
  if (clone)
    player_imu_reset_euler_config_t_copy(clone,msg);
  return clone;
}
void player_imu_reset_euler_config_t_free(player_imu_reset_euler_config_t *msg)
{      
  player_imu_reset_euler_config_t_cleanup(msg);
  free(msg);
}
unsigned int player_imu_reset_euler_config_t_sizeof(player_imu_reset_euler_config_t *msg)
{
  return sizeof(player_imu_reset_euler_config_t);
} 
...
int xdr_player_coopobject_header_t (XDR* xdrs, player_coopobject_header_t * msg)
{   if(xdr_u_char(xdrs,&msg->origin) != 1)
    return(0);
  if(xdr_u_short(xdrs,&msg->id) != 1)
    return(0);
  if(xdr_u_short(xdrs,&msg->parent_id) != 1)
    return(0);
  return(1);
}
int 
player_coopobject_header_pack(void* buf, size_t buflen, player_coopobject_header_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_header_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_header_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_header_t_copy(player_coopobject_header_t *dest, const player_coopobject_header_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_coopobject_header_t));
  return sizeof(player_coopobject_header_t);
} 
void player_coopobject_header_t_cleanup(const player_coopobject_header_t *msg)
{
} 
player_coopobject_header_t * player_coopobject_header_t_clone(const player_coopobject_header_t *msg)
{      
  player_coopobject_header_t * clone = malloc(sizeof(player_coopobject_header_t));
  if (clone)
    player_coopobject_header_t_copy(clone,msg);
  return clone;
}
void player_coopobject_header_t_free(player_coopobject_header_t *msg)
{      
  player_coopobject_header_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_header_t_sizeof(player_coopobject_header_t *msg)
{
  return sizeof(player_coopobject_header_t);
} 

int xdr_player_coopobject_rssi_t (XDR* xdrs, player_coopobject_rssi_t * msg)
{   if(xdr_player_coopobject_header_t(xdrs,&msg->header) != 1)
    return(0);
  if(xdr_u_short(xdrs,&msg->sender_id) != 1)
    return(0);
  if(xdr_u_short(xdrs,&msg->rssi) != 1)
    return(0);
  if(xdr_u_short(xdrs,&msg->stamp) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->nodeTimeHigh) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->nodeTimeLow) != 1)
    return(0);
  if(xdr_float(xdrs,&msg->x) != 1)
    return(0);
  if(xdr_float(xdrs,&msg->y) != 1)
    return(0);
  if(xdr_float(xdrs,&msg->z) != 1)
    return(0);
  return(1);
}
int 
player_coopobject_rssi_pack(void* buf, size_t buflen, player_coopobject_rssi_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_rssi_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_rssi_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_rssi_t_copy(player_coopobject_rssi_t *dest, const player_coopobject_rssi_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_coopobject_rssi_t));
  return sizeof(player_coopobject_rssi_t);
} 
void player_coopobject_rssi_t_cleanup(const player_coopobject_rssi_t *msg)
{
} 
player_coopobject_rssi_t * player_coopobject_rssi_t_clone(const player_coopobject_rssi_t *msg)
{      
  player_coopobject_rssi_t * clone = malloc(sizeof(player_coopobject_rssi_t));
  if (clone)
    player_coopobject_rssi_t_copy(clone,msg);
  return clone;
}
void player_coopobject_rssi_t_free(player_coopobject_rssi_t *msg)
{      
  player_coopobject_rssi_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_rssi_t_sizeof(player_coopobject_rssi_t *msg)
{
  return sizeof(player_coopobject_rssi_t);
} 

int xdr_player_coopobject_position_t (XDR* xdrs, player_coopobject_position_t * msg)
{   if(xdr_player_coopobject_header_t(xdrs,&msg->header) != 1)
    return(0);
  if(xdr_float(xdrs,&msg->x) != 1)
    return(0);
...
int 
player_coopobject_position_pack(void* buf, size_t buflen, player_coopobject_position_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_position_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_position_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_position_t_copy(player_coopobject_position_t *dest, const player_coopobject_position_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_coopobject_position_t));
  return sizeof(player_coopobject_position_t);
} 
void player_coopobject_position_t_cleanup(const player_coopobject_position_t *msg)
{
} 
player_coopobject_position_t * player_coopobject_position_t_clone(const player_coopobject_position_t *msg)
{      
  player_coopobject_position_t * clone = malloc(sizeof(player_coopobject_position_t));
  if (clone)
    player_coopobject_position_t_copy(clone,msg);
  return clone;
}
void player_coopobject_position_t_free(player_coopobject_position_t *msg)
{      
  player_coopobject_position_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_position_t_sizeof(player_coopobject_position_t *msg)
{
  return sizeof(player_coopobject_position_t);
} 

int xdr_player_coopobject_sensor_t (XDR* xdrs, player_coopobject_sensor_t * msg)
{   if(xdr_u_char(xdrs,&msg->type) != 1)
    return(0);
  if(xdr_short(xdrs,&msg->value) != 1)
    return(0);
  return(1);
}
int 
player_coopobject_sensor_pack(void* buf, size_t buflen, player_coopobject_sensor_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_sensor_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_sensor_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_sensor_t_copy(player_coopobject_sensor_t *dest, const player_coopobject_sensor_t *src)
{
  if (dest == NULL || src == NULL)
    return 0;
  memcpy(dest,src,sizeof(player_coopobject_sensor_t));
  return sizeof(player_coopobject_sensor_t);
} 
void player_coopobject_sensor_t_cleanup(const player_coopobject_sensor_t *msg)
{
} 
player_coopobject_sensor_t * player_coopobject_sensor_t_clone(const player_coopobject_sensor_t *msg)
{      
  player_coopobject_sensor_t * clone = malloc(sizeof(player_coopobject_sensor_t));
  if (clone)
    player_coopobject_sensor_t_copy(clone,msg);
  return clone;
}
void player_coopobject_sensor_t_free(player_coopobject_sensor_t *msg)
{      
  player_coopobject_sensor_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_sensor_t_sizeof(player_coopobject_sensor_t *msg)
{
  return sizeof(player_coopobject_sensor_t);
} 

int xdr_player_coopobject_data_sensor_t (XDR* xdrs, player_coopobject_data_sensor_t * msg)
{   if(xdr_player_coopobject_header_t(xdrs,&msg->header) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->data_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
    if((msg->data = malloc(msg->data_count*sizeof(player_coopobject_sensor_t))) == NULL)
      return(0);
  }
  {
    player_coopobject_sensor_t* data_p = msg->data;
    if(xdr_array(xdrs, (char**)&data_p, &msg->data_count, msg->data_count, sizeof(player_coopobject_sensor_t), (xdrproc_t)xdr_player_coopobject_sensor_t) != 1)
      return(0);
  }
  return(1);
}
int 
player_coopobject_data_sensor_pack(void* buf, size_t buflen, player_coopobject_data_sensor_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_data_sensor_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_data_sensor_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_data_sensor_t_copy(player_coopobject_data_sensor_t *dest, const player_coopobject_data_sensor_t *src)
{      
  
  unsigned int size = 0;
  if(src == NULL)
    return(0);
  size += sizeof(player_coopobject_header_t)*1;
  memcpy(&dest->header,&src->header,sizeof(player_coopobject_header_t)*1); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->data_count,&src->data_count,sizeof(uint32_t)*1); 
  if(src->data != NULL && src->data_count > 0)
  {
    if((dest->data = malloc(src->data_count*sizeof(player_coopobject_sensor_t))) == NULL)
      return(0);
  }
  else
    dest->data = NULL;
  size += sizeof(player_coopobject_sensor_t)*src->data_count;
  memcpy(dest->data,src->data,sizeof(player_coopobject_sensor_t)*src->data_count); 
  return(size);
}
void player_coopobject_data_sensor_t_cleanup(const player_coopobject_data_sensor_t *msg)
{      
...
player_coopobject_data_sensor_t * player_coopobject_data_sensor_t_clone(const player_coopobject_data_sensor_t *msg)
{      
  player_coopobject_data_sensor_t * clone = malloc(sizeof(player_coopobject_data_sensor_t));
  if (clone)
    player_coopobject_data_sensor_t_copy(clone,msg);
  return clone;
}
void player_coopobject_data_sensor_t_free(player_coopobject_data_sensor_t *msg)
{      
  player_coopobject_data_sensor_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_data_sensor_t_sizeof(player_coopobject_data_sensor_t *msg)
{
  ...
  size += sizeof(player_coopobject_header_t)*1; 
  size += sizeof(uint32_t)*1; 
  size += sizeof(player_coopobject_sensor_t)*msg->data_count; 
  return(size);
}

int xdr_player_coopobject_data_userdefined_t (XDR* xdrs, player_coopobject_data_userdefined_t * msg)
{   if(xdr_player_coopobject_header_t(xdrs,&msg->header) != 1)
    return(0);
  if(xdr_u_char(xdrs,&msg->type) != 1)
    return(0);
...
int 
player_coopobject_data_userdefined_pack(void* buf, size_t buflen, player_coopobject_data_userdefined_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_data_userdefined_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_data_userdefined_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_data_userdefined_t_copy(player_coopobject_data_userdefined_t *dest, const player_coopobject_data_userdefined_t *src)
{      
  ...
  memcpy(&dest->header,&src->header,sizeof(player_coopobject_header_t)*1); 
  size += sizeof(uint8_t)*1;
  memcpy(&dest->type,&src->type,sizeof(uint8_t)*1); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->data_count,&src->data_count,sizeof(uint32_t)*1); 
  if(src->data != NULL && src->data_count > 0)
  {
...
void player_coopobject_data_userdefined_t_cleanup(const player_coopobject_data_userdefined_t *msg)
{      
...
player_coopobject_data_userdefined_t * player_coopobject_data_userdefined_t_clone(const player_coopobject_data_userdefined_t *msg)
{      
  player_coopobject_data_userdefined_t * clone = malloc(sizeof(player_coopobject_data_userdefined_t));
  if (clone)
    player_coopobject_data_userdefined_t_copy(clone,msg);
  return clone;
}
void player_coopobject_data_userdefined_t_free(player_coopobject_data_userdefined_t *msg)
{      
  player_coopobject_data_userdefined_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_data_userdefined_t_sizeof(player_coopobject_data_userdefined_t *msg)
{
...
int xdr_player_coopobject_req_t (XDR* xdrs, player_coopobject_req_t * msg)
{   if(xdr_player_coopobject_header_t(xdrs,&msg->header) != 1)
    return(0);
  if(xdr_u_char(xdrs,&msg->request) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->parameters_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
  {
    if((msg->parameters = malloc(msg->parameters_count*sizeof(uint8_t))) == NULL)
      return(0);
  }
  {
    uint8_t* parameters_p = msg->parameters;
    if(xdr_bytes(xdrs, (char**)&parameters_p, &msg->parameters_count, msg->parameters_count) != 1)
      return(0);
  }
  return(1);
}
int 
player_coopobject_req_pack(void* buf, size_t buflen, player_coopobject_req_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_req_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_req_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_req_t_copy(player_coopobject_req_t *dest, const player_coopobject_req_t *src)
{      
  ...
  memcpy(&dest->header,&src->header,sizeof(player_coopobject_header_t)*1); 
  size += sizeof(uint8_t)*1;
  memcpy(&dest->request,&src->request,sizeof(uint8_t)*1); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->parameters_count,&src->parameters_count,sizeof(uint32_t)*1); 
  if(src->parameters != NULL && src->parameters_count > 0)
  {
    if((dest->parameters = malloc(src->parameters_count*sizeof(uint8_t))) == NULL)
      return(0);
  }
  else
    dest->parameters = NULL;
  size += sizeof(uint8_t)*src->parameters_count;
  memcpy(dest->parameters,src->parameters,sizeof(uint8_t)*src->parameters_count); 
  return(size);
}
void player_coopobject_req_t_cleanup(const player_coopobject_req_t *msg)
{      
...
player_coopobject_req_t * player_coopobject_req_t_clone(const player_coopobject_req_t *msg)
{      
  player_coopobject_req_t * clone = malloc(sizeof(player_coopobject_req_t));
  if (clone)
    player_coopobject_req_t_copy(clone,msg);
  return clone;
}
void player_coopobject_req_t_free(player_coopobject_req_t *msg)
{      
  player_coopobject_req_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_req_t_sizeof(player_coopobject_req_t *msg)
{
  ...
  size += sizeof(uint8_t)*1; 
  size += sizeof(uint32_t)*1; 
  size += sizeof(uint8_t)*msg->parameters_count; 
  return(size);
}

int xdr_player_coopobject_cmd_t (XDR* xdrs, player_coopobject_cmd_t * msg)
{   if(xdr_player_coopobject_header_t(xdrs,&msg->header) != 1)
    return(0);
  if(xdr_u_char(xdrs,&msg->command) != 1)
    return(0);
  if(xdr_u_int(xdrs,&msg->parameters_count) != 1)
    return(0);
  if(xdrs->x_op == XDR_DECODE)
...
int 
player_coopobject_cmd_pack(void* buf, size_t buflen, player_coopobject_cmd_t * msg, int op)
{
  XDR xdrs;
  int len;
  if(!buflen)
    return 0;
  xdrmem_create(&xdrs, buf, buflen, op);
  if(xdr_player_coopobject_cmd_t(&xdrs,msg) != 1)
    return(-1);
  if(op == PLAYERXDR_ENCODE)
    len = xdr_getpos(&xdrs);
  else
    len = sizeof(player_coopobject_cmd_t);
  xdr_destroy(&xdrs);
  return(len);
} 
unsigned int player_coopobject_cmd_t_copy(player_coopobject_cmd_t *dest, const player_coopobject_cmd_t *src)
{      
  ...
  memcpy(&dest->header,&src->header,sizeof(player_coopobject_header_t)*1); 
  size += sizeof(uint8_t)*1;
  memcpy(&dest->command,&src->command,sizeof(uint8_t)*1); 
  size += sizeof(uint32_t)*1;
  memcpy(&dest->parameters_count,&src->parameters_count,sizeof(uint32_t)*1); 
  if(src->parameters != NULL && src->parameters_count > 0)
  {
...
void player_coopobject_cmd_t_cleanup(const player_coopobject_cmd_t *msg)
{      
...
player_coopobject_cmd_t * player_coopobject_cmd_t_clone(const player_coopobject_cmd_t *msg)
{      
  player_coopobject_cmd_t * clone = malloc(sizeof(player_coopobject_cmd_t));
  if (clone)
    player_coopobject_cmd_t_copy(clone,msg);
  return clone;
}
void player_coopobject_cmd_t_free(player_coopobject_cmd_t *msg)
{      
  player_coopobject_cmd_t_cleanup(msg);
  free(msg);
}
unsigned int player_coopobject_cmd_t_sizeof(player_coopobject_cmd_t *msg)
{
